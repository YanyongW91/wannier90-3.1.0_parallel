...
subroutine plot_main()
  ...
  use w90_io,         only: stdout, io_stopwatch, io_time
  use w90_parameters, only: num_kpts, bands_plot, dos_plot, kpt_latt, fermi_surface_plot, wannier_plot, timing_level, write_bvec, write_hr, write_rmn, write_tb, write_u_matrices
  use w90_hamiltonian,only: hamiltonian_get_hr, hamiltonian_write_hr, hamiltonian_setup, hamiltonian_write_rmn, hamiltonian_write_tb, nrpts, irvec
  ...
  if(on_root)  then
    if( timing_level .gt.0)  call io_stopwatch('plot: main', 1)
  
    ! Print the header only if there is something to plot
    if( bands_plot .or. dos_plot .or. fermi_surface_plot .or. write_hr .or. wannier_plot .or. write_u_matrices .or. write_tb)  then
      write( stdout, '(/1x,a)') '*---------------------------------------------------------------------------*'
      write( stdout, '(1x,a)') '|                               PLOTTING                                    |'
      write( stdout, '(1x,a/)') '*---------------------------------------------------------------------------*'
    endif
    
    if( bands_plot .or. dos_plot .or. fermi_surface_plot .or. write_hr .or. write_tb)  then
      ! Check if the kmesh includes the gamma point
      have_gamma= .false.
      do nkp= 1, num_kpts
         if( all( abs( kpt_latt(:, nkp) ) < eps6) ) have_gamma= .true.
      enddo
      if( .not.have_gamma)  write( stdout, '(1x,a)') '!!!! Kpoint grid does not include Gamma. '// &
  ...
  if(on_root)  then
    ...
    if( timing_level .gt.0)  call io_stopwatch('plot: main', 2)
  endif
...
subroutine plot_interpolate_bands
  ...
  if( timing_level .gt.1)  call io_stopwatch('plot: interpolate_bands', 1)
  time0= io_time()
  write( stdout, *)
  write( stdout, '(1x,a)') 'Calculating interpolated band-structure'
  write( stdout, *)
  allocate( ham_pack( (num_wann*( num_wann +1) ) /2), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating ham_pack in plot_interpolate_bands')
  allocate( ham_kprm( num_wann, num_wann), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating ham_kprm in plot_interpolate_bands')
  allocate( U_int( num_wann, num_wann), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating U_int in plot_interpolate_bands')
  allocate( cwork(2*num_wann), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating cwork in plot_interpolate_bands')
  allocate( rwork(7*num_wann), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating rwork in plot_interpolate_bands')
  allocate( iwork(5*num_wann), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating iwork in plot_interpolate_bands')
  allocate( ifail( num_wann), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating ifail in plot_interpolate_bands')

  allocate( idx_special_points(bands_num_spec_points), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating idx_special_points in plot_interpolate_bands')
  allocate( xval_special_points(bands_num_spec_points), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating xval_special_points in plot_interpolate_bands')
  ...
  do i= 2, num_paths
    ...
    if( ( sUM( (bands_spec_points(:, (i -1)*2) - bands_spec_points(:, (i -1)*2 +1) )**2) .gt.1.e-6) .or. &
  ...
  if( ierr.ne.0)  call io_error('Error in allocating plot_kpoint in plot_interpolate_bands')
  allocate( xval(total_pts), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating xval in plot_interpolate_bands')
  allocate( eig_int( num_wann, total_pts), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating eig_int in plot_interpolate_bands')
  allocate( bands_proj( num_wann, total_pts), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating bands_proj in plot_interpolate_bands')
  allocate( glabel( num_spts), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating num_spts in plot_interpolate_bands')
  allocate( xlabel( num_spts), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating xlabel in plot_interpolate_bands')
  allocate( ctemp(bands_num_spec_points), stat=ierr)
  if( ierr.ne.0)  call io_error('Error in allocating ctemp in plot_interpolate_bands')
...
!~[aam] check divide by ndegen?
      ...
      if( num_bands_project .gt.0)  then
    ...
    if( index(bands_plot_format, 'gnu') .gt.0)  call plot_interpolate_gnuplot
    if( index(bands_plot_format, 'xmgr') .gt.0)  call plot_interpolate_xmgrace
    ...
    if( ierr.ne.0)  call io_error('Error in deallocating ham_r_cut in plot_interpolate_bands')
    if( allocated(irvec_cut) ) deallocate( irvec_cut, stat=ierr)
    if( ierr.ne.0)  call io_error('Error in deallocating irvec_cut in plot_interpolate_bands')
    if( timing_level .gt.1)  call io_stopwatch('plot: interpolate_bands', 2)
    if( allocated(idx_special_points) ) deallocate( idx_special_points, stat=ierr)
    if( ierr.ne.0)  call io_error('Error in deallocating idx_special_points in plot_interpolate_bands')
    if( allocated(xval_special_points) ) deallocate( xval_special_points, stat=ierr)
    if( ierr.ne.0)  call io_error('Error in deallocating xval_special_points in plot_interpolate_bands')
  contains
    ...
    subroutine plot_cut_hr()
      ...
      if( ierr.ne.0)  call io_error('Error in allocating ham_r_tmp in plot_cut_hr')
      ...
      if( ierr.ne.0)  call io_error('Error in allocating ham_r_cut in plot_cut_hr')
      allocate( irvec_cut(3, nrpts_cut), stat=ierr)
      if( ierr.ne.0)  call io_error('Error in allocating irvec_cut in plot_cut_hr')
      allocate( shift_vec(3, nrpts_cut), stat=ierr)
      if( ierr.ne.0)  call io_error('Error in allocating shift_vec in plot_cut_hr')
      ...
      if( index( dist_cutoff_mode, 'one_dim') .gt.0)  then
        do i= 1, num_wann
          ...
          enddo
        enddo
      else if( index( dist_cutoff_mode, 'two_dim') .gt.0)  then
        do i= 1, num_wann
          do j= 1, num_wann
...
705   format('set arrow from ', F8.5, ',', F10.5, ' to ', F8.5, ',', F10.5, ' nohead')
  ...
  subroutine plot_fermi_surface
    ...
    character( len=9)     :: cdate, ctime
    !
    if( timing_level .gt.1)  call io_stopwatch('plot: fermi_surface', 1)
    time0= io_time()
    write( stdout, *)
    write( stdout, '(1x,a)') 'Calculating Fermi surface'
    write( stdout, *)
    !
    if( nfermi .gt.1)  call io_error("Error in plot: nfermi>1. Set the fermi level " &
                                  //"using the input parameter 'fermi_level'")
    !
    allocate( ham_pack( (num_wann*( num_wann +1) ) /2), stat=ierr)
    if( ierr.ne.0)  call io_error('Error in allocating ham_pack plot_fermi_surface')
    allocate( ham_kprm( num_wann, num_wann), stat=ierr)
    if( ierr.ne.0)  call io_error('Error in allocating ham_kprm plot_fermi_surface')
    allocate( U_int( num_wann, num_wann), stat=ierr)
    if( ierr.ne.0)  call io_error('Error in allocating U_int in plot_fermi_surface')
    allocate( cwork(2*num_wann), stat=ierr)
    if( ierr.ne.0)  call io_error('Error in allocating cwork in plot_fermi_surface')
    allocate( rwork(7*num_wann), stat=ierr)
    if( ierr.ne.0)  call io_error('Error in allocating rwork in plot_fermi_surface')
    allocate( iwork(5*num_wann), stat=ierr)
    if( ierr.ne.0)  call io_error('Error in allocating iwork in plot_fermi_surface')
    allocate( ifail( num_wann), stat=ierr)
    if( ierr.ne.0)  call io_error('Error in allocating ifail in plot_fermi_surface')
    !
    npts_plot= (fermi_surface_num_points +1)**3
    allocate( eig_int( num_wann, npts_plot), stat=ierr)
    if( ierr.ne.0)  call io_error('Error in allocating eig_int in plot_fermi_surface')
    eig_int= 0.0_dp
    U_int= (0.0_dp, 0.0_dp)
    !
    ikp= 0
    ...
    write( stdout, *)
    !
    if( timing_level .gt.1)  call io_stopwatch('plot: fermi_surface', 2)
    !
    return
...
subroutine plot_wannier
  ...
  use w90_comms, only: my_node_id, num_nodes, comms_reduce, on_root, comms_bcast, comms_barrier
  ! use w90_comms
  ...
  complex( kind=dp), allocatable :: wann_func(:,:,:,:), wann_func_tmp(:,:,:,:), r_wvfn(:,:), r_wvfn_tmp(:,:)
  ...
  logical :: have_file
  integer :: i, j, nsp, nat, nbnd, counter, ierr, loop_kpt, ik, ix, iy, iz, nk, ngx, ngy, ngz, &
             nxx, nyy, nzz, loop_b, nx, ny, nz, npoint, file_unit, loop_w, num_inc, ispinor
  character( len=11) :: wfnname
  character( len=60) :: wanxsf, wancube
  character( len=9)  :: cdate, ctime
  logical            :: inc_band( num_bands)
  integer :: mpierr, k_idx_loc, size_wann_func
200 format('UNK', i5.5, '.', i1)
199 format('UNK', i5.5, '.', 'NC')

  if(on_root)  then
    if( timing_level .gt.1)  call io_stopwatch('plot: wannier', 1)
    if( .not.spinors)  then
  ...
  call comms_bcast( ngx, 1)
  call comms_bcast( ngy, 1)
  call comms_bcast( ngz, 1)
  call comms_bcast( nk, 1)
  call comms_bcast( nbnd, 1)
  ...
  if( ierr.ne.0)  call io_error('Error in allocating wann_func in plot_wannier')
  wann_func= cmplx_0
  if( spinors)  then
    allocate( wann_func_nc(-( ngs(1) /2) *ngx:( (ngs(1) +1) /2) *ngx -1, &
                           -( ngs(2) /2) *ngy:( (ngs(2) +1) /2) *ngy -1, &
                           -( ngs(3) /2) *ngz:( (ngs(3) +1) /2) *ngz -1, 2, num_wannier_plot), stat=ierr)
    if( ierr.ne.0)  call io_error('Error in allocating wann_func_nc in plot_wannier')
  ...
  if( .not.spinors)  then
    if( have_disentangled)  then
      ...
      if( ierr.ne.0)  call io_error('Error in allocating r_wvfn_tmp in plot_wannier')
    endif
    allocate( r_wvfn( ngx*ngy*ngz, num_wann), stat=ierr)
    if( ierr.ne.0)  call io_error('Error in allocating r_wvfn in plot_wannier')
  else
    if( have_disentangled)  then
      allocate( r_wvfn_tmp_nc( ngx*ngy*ngz, maxval( ndimwin), 2), stat=ierr)
      if( ierr.ne.0)  call io_error('Error in allocating r_wvfn_tmp_nc in plot_wannier')
    endif
    allocate( r_wvfn_nc( ngx*ngy*ngz, num_wann, 2), stat=ierr)
    if( ierr.ne.0)  call io_error('Error in allocating r_wvfn_nc in plot_wannier')
  endif

  call io_date( cdate, ctime)
  if(on_root)  then
    if( on_root) write(stdout, '(A18,I6)') 'num_wannier_plot:', num_wannier_plot
    write(stdout, '(A18,I6)') 'num_nodes:', num_nodes
    write(stdout, '(A18,I6)') 'num_kpts:', num_kpts
    write(stdout, '(A18,I6)') 'blk_size:', num_kpts /num_nodes +1
  endif
  k_idx_loc=0
  do loop_kpt= 1, num_kpts
#ifdef MPI
     if( mod( loop_kpt, num_nodes) .ne.my_node_id)  cycle
     k_idx_loc= k_idx_loc +1
     if( on_root) write(stdout,'(1X,A,I6)') '| k_idx_loc', k_idx_loc
#endif
     ...
     if( have_disentangled)  then
       ...
       if( .not.spinors)  then
         ...
         do loop_w= 1, num_wann
            do loop_b= 1, num_inc
               r_wvfn(:, loop_w)= r_wvfn(:, loop_w) +u_matrix_opt( loop_b, loop_w, loop_kpt)*r_wvfn_tmp(:, loop_b)
            enddo
         enddo
         ...
         do loop_w= 1, num_wann
            do loop_b= 1, num_inc
               call zaxpy( ngx*ngy*ngz, u_matrix_opt( loop_b, loop_w, loop_kpt), r_wvfn_tmp_nc(1, loop_b, 1), 1, r_wvfn_nc(1, loop_w, 1), 1) ! up-spinor
               call zaxpy( ngx*ngy*ngz, u_matrix_opt( loop_b, loop_w, loop_kpt), r_wvfn_tmp_nc(1, loop_b, 2), 1, r_wvfn_nc(1, loop_w, 2), 1) ! down-spinor
            enddo
         enddo
       endif
     endif
     ...
     ! nx ny nz are the nxx nyy nzz brought back to the unit cell in which u_nk(r)=cptwrb(r,n)  is represented
     ! There is a big performance improvement in looping over num_wann in the inner loop. This is poor memory access for wann_func and
     ...
     enddo ! ends nzz
  enddo ! ends loop_kpt
  if( allocated( wann_func_nc) ) deallocate( wann_func_nc)
#ifdef MPI
  size_wann_func= ngs(1)*ngx*ngs(2)*ngy*ngs(3)*ngz*num_wannier_plot
  allocate( wann_func_tmp( ngs(1)*ngx, ngs(2)*ngy, ngs(3)*ngz, num_wannier_plot ) )
  call mpi_allreduce( wann_func, wann_func_tmp, ngs(1)*ngx*ngs(2)*ngy*ngs(3)*ngz*num_wannier_plot, mpi_double_complex, mpi_sum, mpi_comm_world, mpierr)
  ! wann_func= wann_func_tmp
  call zcopy( size_wann_func, wann_func_tmp, 1, wann_func, 1)
  deallocate( wann_func_tmp)
#endif
  ...
  if( .not.spinors)  then !!!!! For spinor Wannier functions, the steps below are not necessary.
    ...
    wann_func= wann_func /real( num_kpts, dp)
    do loop_w= 1, num_wannier_plot
       if( mod( loop_w, num_nodes) .ne.my_node_id)  cycle
       tmaxx= 0.0
       wmod= cmplx_1
       do nzz= -( ngs(3) /2) *ngz, ( (ngs(3) +1) /2) *ngz -1
          do nyy= -( ngs(2) /2) *ngy, ( (ngs(2) +1) /2) *ngy -1
          ...
          do nyy= -( ngs(2) /2) *ngy, ( (ngs(2) +1) /2) *ngy -1
             do nxx= -( ngs(1) /2) *ngx, ( (ngs(1) +1) /2) *ngx -1
                if( abs( real( wann_func( nxx, nyy, nzz, loop_w), dp) ) >= 0.01_dp)  then
                  ratio = abs( aimag( wann_func( nxx, nyy, nzz, loop_w) ) ) / abs( real( wann_func( nxx, nyy, nzz, loop_w), dp) )
                  ratmax= max(ratmax, ratio)
                endif
             enddo ! ends nxx
          enddo ! ends nyy
       enddo ! ends nzz
       write(stdout, '(6x,a,i4,7x,a,f11.6)') 'Wannier Function Num: ', wannier_plot_list( loop_w), 'Maximum Im/Re Ratio= ', ratmax
    enddo ! ends loop_w
  endif ! endif spinors

  if( wannier_plot_format .eq. 'xcrysden')  then
    if( on_root)  write(stdout, '(1X,A)') 'writing to .xsf files....'
    call internal_xsf_format()
    elseif( wannier_plot_format .eq. 'cube')  then
      if( on_root)  call internal_cube_format()
    else
      call io_error('wannier_plot_format not recognised in wannier_plot')
    endif
    if( timing_level .gt.1)  call io_stopwatch('plot: wannier', 2)
...
contains
  ...
  subroutine internal_cube_format()
    ...
    lmol= .false.
    lcrys= .false.
    if( index( wannier_plot_mode, 'mol') .gt.0) lmol= .true.      ! molecule mode
    if( index( wannier_plot_mode, 'crys') .gt.0) lcrys= .true.    ! crystal mode
...
!                if( qxx.gt.( ngs(1)-1) *ngx-1)  then
      ...
      if( lmol)  then ! In "molecule mode" translate origin of cube to bring it in coincidence with the atomic positions
        ...
        if( iprint .gt.3) write( stdout, '(a,3f12.6,/)') 'orig-new=', (orig(i), i=1, 3)
        ...
        do isp= 1, num_species
          ...
          enddo ! iat
        enddo ! isp
        if( iprint .gt.3) write( stdout, '(a,i12)') 'icount =', icount
      endif
...
subroutine internal_xsf_format()
  ! use w90_comms, only: num_nodes, my_node_id
  ! use w90_parameters, only: num_wann, num_bands, num_kpts, u_matrix, spin, &
  !   ngs=> wannier_plot_supercell, kpt_latt, num_species, atoms_species_num, &
  implicit none
...
!  integer, allocatable:: atoms_species_num_tmp(:)
  ! allocate( atoms_species_num_tmp( num_species) )
  if( .not. allocated( atoms_species_num) )  allocate( atoms_species_num( num_species) )
  call comms_bcast( atoms_species_num(1), num_species)
  if( .not. allocated( atoms_symbol) )  allocate( atoms_symbol( num_species) )
  call comms_bcast( atoms_symbol(1), num_species*2)
  if( .not. allocated( ATOMS_POS_CART) )  allocate( ATOMS_POS_CART(3, maxval( atoms_species_num), num_species) )
  call comms_bcast( ATOMS_POS_CART(1,1,1), 3*maxval( atoms_species_num)*num_species)
  ...
  do loop_b= 1, num_wannier_plot
     if( mod( loop_b, num_nodes) .ne.my_node_id)  cycle
     ...
     if( index( wannier_plot_mode, 'mol') .gt.0)  then
